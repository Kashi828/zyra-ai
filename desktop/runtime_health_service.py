...
class RuntimeHealthService:
    """Local runtime health checks. Does not mutate configuration or install anything."""
    ...
    def read(self, *, api: str = "online", voice: str = "unconfigured") -> dict[str, str]:
        model = self.check_model()
        runtime = "ready" if api == "online" and model in {"online", "configured"} else "degraded"
        return asdict(RuntimeHealth(api=api, model=model, voice=voice, runtime=runtime))
