...
def test_session_inventory_defaults_to_active_only(tmp_path):
    ...
    assert response.status_code == 200
    body = response.json()
    assert body["summary"] == {"total": 1, "active": 1, "inactive": 0}
    assert [item["session_id"] for item in body["sessions"]] == [current]


def test_session_inventory_can_include_inactive_history(tmp_path):
    app = create_app(db_path=str(tmp_path / "security.sqlite3"))
    secret = _enroll(app.state.security_context.store)
    client = TestClient(app)
    current = _create_session(client, secret)["session_id"]
    ...
    assert sessions[current]["current"] is True
    assert sessions[current]["active"] is True
    assert sessions[target]["current"] is False
...
def test_session_inventory_never_crosses_device_boundary(tmp_path):
    ...
    assert foreign not in returned_ids
    assert response.json()["device"]["device_id"] == "dev-one"
