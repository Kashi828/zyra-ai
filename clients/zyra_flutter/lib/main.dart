...
class ZyraApp extends StatelessWidget {
  ...
  Widget build(BuildContext context) => MaterialApp(
        ...
        title: 'ZYRA AI',
        theme: ThemeData(brightness: Brightness.dark, useMaterial3: true, scaffoldBackgroundColor: const Color(0xFF07080D), colorScheme: ColorScheme.fromSeed(seedColor: const Color(0xFF8F7CFF), brightness: Brightness.dark)),
        home: const ZyraHome(),
      );
}
...
class _ZyraHomeState extends State<ZyraHome> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      bottomNavigationBar: wide ? null : NavigationBar(selectedIndex: tab.index, onDestinationSelected: (i) => setState(() => tab = ZyraTab.values[i]), destinations: const [
        NavigationDestination(icon: Icon(Icons.auto_awesome), label: 'Home'), NavigationDestination(icon: Icon(Icons.bolt_outlined), label: 'Activity'), NavigationDestination(icon: Icon(Icons.devices_outlined), label: 'Devices'), NavigationDestination(icon: Icon(Icons.tune_outlined), label: 'Settings')]),
    );
  }

  Widget _page(bool wide) => AnimatedSwitcher(duration: const Duration(milliseconds: 260), child: switch (tab) {
...
class _Rail extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) => Container(width: 82, margin: const EdgeInsets.all(14), decoration: BoxDecoration(color: const Color(0xFF0D0F16), borderRadius: BorderRadius.circular(26), border: Border.all(color: Colors.white.withOpacity(.07))), child: Column(children: [const SizedBox(height: 22), const _Mark(), const SizedBox(height: 30),
    for (final value in ZyraTab.values) Padding(padding: const EdgeInsets.symmetric(vertical: 5), child: IconButton.filledTonal(isSelected: value == tab, onPressed: () => onTab(value), icon: Icon(switch (value) { ZyraTab.home => Icons.auto_awesome, ZyraTab.activity => Icons.bolt_outlined, ZyraTab.devices => Icons.devices_outlined, ZyraTab.settings => Icons.tune_outlined }))),
...
class _Home extends StatelessWidget {
  ...
  Widget build(BuildContext context) => CustomScrollView(slivers: [SliverPadding(padding: EdgeInsets.fromLTRB(wide ? 38 : 22, 24, wide ? 38 : 22, 40), sliver: SliverList(delegate: SliverChildListDelegate([
    Row(children: [const Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text('Good to see you', style: TextStyle(color: Colors.white54)), SizedBox(height: 4), Text('ZYRA', style: TextStyle(fontSize: 31, fontWeight: FontWeight.w700))])), _Pill(text: 'Protected')]),
    const SizedBox(height: 36), Center(child: Column(children: [AnimatedContainer(duration: const Duration(milliseconds: 260), width: listening ? 166 : 146, height: listening ? 166 : 146, decoration: BoxDecoration(shape: BoxShape.circle, gradient: RadialGradient(colors: [const Color(0xFFB7A6FF).withOpacity(.95), const Color(0xFF6E5DE7).withOpacity(.4), Colors.transparent]), boxShadow: [BoxShadow(color: const Color(0xFF8B7CFF).withOpacity(.22), blurRadius: 45, spreadRadius: 8)]), child: Center(child: Container(width: 84, height: 84, decoration: BoxDecoration(shape: BoxShape.circle, color: const Color(0xFF0D0F16), border: Border.all(color: Colors.white.withOpacity(.12))), child: const Icon(Icons.auto_awesome, size: 34)))), const SizedBox(height: 18), Text(listening ? 'Listening…' : 'How can I help?', style: const TextStyle(fontSize: 24, fontWeight: FontWeight.w700)), const SizedBox(height: 6), Text(listening ? 'Speak naturally. Your permissions still apply.' : 'One assistant across your phone and PC.', textAlign: TextAlign.center, style: const TextStyle(color: Colors.white54))])),
...
class _Actions extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    const items = [(Icons.apps_outlined, 'Open app'), (Icons.folder_open_outlined, 'Find files'), (Icons.language_outlined, 'Browse'), (Icons.more_horiz, 'More')];
...
class _Ecosystem extends StatelessWidget {
  const _Ecosystem();
  @override
  Widget build(BuildContext context) => Container(padding: const EdgeInsets.all(18), decoration: BoxDecoration(color: const Color(0xFF11141D), borderRadius: BorderRadius.circular(24), border: Border.all(color: Colors.white.withOpacity(.07))), child: const Column(children: [_Device(icon: Icons.smartphone_rounded, title: 'This phone', subtitle: 'Active session'), SizedBox(height: 14), _Device(icon: Icons.desktop_windows_rounded, title: 'Windows PC', subtitle: 'Ready for secure tasks')]));
}

class _Device extends StatelessWidget { const _Device({required this.icon, required this.title, required this.subtitle}); final IconData icon; final String title; final String subtitle; @override Widget build(BuildContext context) => Row(children: [Container(width: 46, height: 46, decoration: BoxDecoration(color: Colors.white.withOpacity(.05), borderRadius: BorderRadius.circular(15)), child: Icon(icon)), const SizedBox(width: 13), Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(title, style: const TextStyle(fontWeight: FontWeight.w600)), const SizedBox(height: 4), Text(subtitle, style: const TextStyle(color: Colors.white54, fontSize: 12))])), const _Dot(), const SizedBox(width: 7), const Text('Online', style: TextStyle(fontSize: 12, color: Colors.white70))]); }

class _Recent extends StatelessWidget { const _Recent(); @override Widget build(BuildContext context) { const items = [('Opened browser', 'Windows PC · just now'), ('Checked connection', 'Windows PC · 8 min ago'), ('Paired device', 'Android · yesterday')]; return Container(decoration: BoxDecoration(color: const Color(0xFF11141D), borderRadius: BorderRadius.circular(22)), child: Column(children: [for (var i = 0; i < items.length; i++) ...[ListTile(leading: const CircleAvatar(child: Icon(Icons.history_rounded, size: 18)), title: Text(items[i].$1, style: const TextStyle(fontSize: 14, fontWeight: FontWeight.w600)), subtitle: Text(items[i].$2, style: const TextStyle(color: Colors.white54, fontSize: 12))), if (i != items.length - 1) Divider(height: 1, color: Colors.white.withOpacity(.05))]])); }}

class _InfoPage extends StatelessWidget { const _InfoPage({required this.title, required this.icon, required this.body, super.key}); final String title, body; final IconData icon; @override Widget build(BuildContext context) => Center(child: Padding(padding: const EdgeInsets.all(28), child: Column(mainAxisAlignment: MainAxisAlignment.center, children: [Container(width: 76, height: 76, decoration: BoxDecoration(color: Colors.white.withOpacity(.06), shape: BoxShape.circle), child: Icon(icon, size: 32)), const SizedBox(height: 22), Text(title, style: const TextStyle(fontSize: 30, fontWeight: FontWeight.w700)), const SizedBox(height: 10), Text(body, textAlign: TextAlign.center, style: const TextStyle(color: Colors.white54, height: 1.5))]))); }

class _Title extends StatelessWidget { const _Title(this.text); final String text; @override Widget build(BuildContext context) => Text(text, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w650)); }
...
class _Mark extends StatelessWidget { const _Mark(); @override Widget build(BuildContext context) => Container(width: 44, height: 44, decoration: BoxDecoration(color: Colors.white.withOpacity(.07), borderRadius: BorderRadius.circular(15)), child: const Icon(Icons.auto_awesome)); }
