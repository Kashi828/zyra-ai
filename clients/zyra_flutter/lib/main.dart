  ...
  Widget _page(bool wide) {
    return AnimatedSwitcher(
      ...
      child: switch (tab) {
        ...
        ZyraTab.activity => const _InfoPage(
            key: ValueKey('activity'),
            title: 'Activity',
            icon: Icons.bolt_outlined,
            body: 'Your ZYRA task timeline, progress, and protected actions.',
          ),
        ZyraTab.devices => const _InfoPage(
            key: ValueKey('devices'),
            title: 'Devices',
            icon: Icons.devices_outlined,
            body: 'Trusted devices, sessions, pairing, and permissions.',
          ),
        ZyraTab.settings => const _InfoPage(
            key: ValueKey('settings'),
            title: 'Settings',
            icon: Icons.tune_outlined,
            body: 'Appearance, privacy, notifications, and advanced controls.',
          ),
      },
    );
  }
...
class _Rail extends StatelessWidget {
  ...
  IconData _tabIcon(ZyraTab value) => switch (value) {
        ZyraTab.home => Icons.auto_awesome,
        ZyraTab.activity => Icons.bolt_outlined,
        ZyraTab.devices => Icons.devices_outlined,
        ZyraTab.settings => Icons.tune_outlined,
      };
}
              ...
              Center(
                child: Column(
                  children: [
                    AnimatedContainer(
                      duration: const Duration(milliseconds: 260),
                      width: listening ? 166 : 146,
                      height: listening ? 166 : 146,
                      decoration: BoxDecoration(
                        shape: BoxShape.circle,
                        gradient: RadialGradient(
                          colors: [
                            const Color(0xFFB7A6FF).withOpacity(.95),
                            const Color(0xFF6E5DE7).withOpacity(.4),
                            Colors.transparent,
                          ],
                        ),
                        boxShadow: [
                          BoxShadow(
                            color: const Color(0xFF8B7CFF).withOpacity(.22),
                            blurRadius: 45,
                            spreadRadius: 8,
                          ),
                        ],
                      ...
                      child: Center(
                        child: Container(
                          ...
                          child: const Icon(Icons.auto_awesome, size: 34),
                        ),
                      ),
                    ),
                    ...
                    Text(
                      listening
                          ? 'Speak naturally. Your permissions still apply.'
                          : 'One assistant across your phone and PC.',
    ...
    return Container(
      ...
      child: Row(
        children: [
          ...
          FilledButton(onPressed: onRun, child: const Icon(Icons.arrow_upward)),
        ],
      ),
    );
  }
...
class _Actions extends StatelessWidget {
  const _Actions({required this.wide});
  ...
  Widget build(BuildContext context) {
    const items = [
      (Icons.apps_outlined, 'Open app'),
      (Icons.folder_open_outlined, 'Find files'),
      (Icons.language_outlined, 'Browse'),
        ...
        child: InkWell(
          ...
          child: Padding(
            ...
            child: Column(
              ...
              children: [
                Icon(items[index].$1, size: 25),
                const SizedBox(height: 13),
                Text(items[index].$2, style: const TextStyle(fontWeight: FontWeight.w600)),
              ],
            ),
          ),
        ),
...
class _Ecosystem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Container(
      ...
      child: const Column(
        children: [
          _Device(icon: Icons.smartphone_rounded, title: 'This phone', subtitle: 'Active session'),
          SizedBox(height: 14),
          _Device(icon: Icons.desktop_windows_rounded, title: 'Windows PC', subtitle: 'Ready for secure tasks'),
        ],
      ),
    );
  }
...
class _Recent extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    const items = [
      ('Opened browser', 'Windows PC · just now'),
      ('Checked connection', 'Windows PC · 8 min ago'),
      ('Paired device', 'Android · yesterday'),
    ];
...
class _Title extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    return Text(text, style: const TextStyle(fontSize: 17, fontWeight: FontWeight.w600));
  }
}
    ...
    return Container(
      ...
      child: const Row(
        ...
        children: [
          _Dot(),
          SizedBox(width: 8),
          Text('Protected', style: TextStyle(fontSize: 13, fontWeight: FontWeight.w600)),
        ],
      ),
    );
  }
